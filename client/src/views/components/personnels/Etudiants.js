...
const Etudiants = () => {
    ...
    const onSendShow = id => {
      setLoading(id)
      history.push({
        pathname: '/personnels/inscription/profile',
        state: { 
                ...
                <CTableBody className="">
                  {etudiants.length>0 ? etudiants.map((etudiant,index) => (
                    ...
                    >
                      ...
                      <CTableDataCell className="text-center">
                        <CBadge color={etudiant.etudiant.statut_etudiant ? "success" : "danger"}>{etudiant.etudiant.statut_etudiant ? 'Validé' : 'Non validée'}</CBadge>
                      </CTableDataCell>
