...
const Reinscription = () => {
    ...
    const [currEtudiant, setCurrEtudiant] = useState({})
    const [currInscription, setCurrInscription] = useState([])
    ...
    const saveInscription = () => {
        ...
        formData.append("upload_quitus", quitus[0].upload_quitus)
    ...
    const setReInscription = () => {
        const user = JSON.parse(localStorage.getItem("user"))
        setCurrEtudiant(user)

        EtudiantService.findByIdInscriptionAll(user.num_inscription_etudiant,1)
        .then(response => {
  ...
  return (
    <>
      ...
      {currInscription.length > 0 && (
      <CRow>
        <CCol xs>
          <CCard className="mb-4">
            <CCardHeader className="pt-4 pb-3 text-center"> <CCardTitle> VOTRE RE-INSCRIPTION </CCardTitle> </CCardHeader>
                <CCardBody>
                <CTable hover responsive align="middle" className="mb-0">
                        <CTableHead color="dark">
                            <CTableRow>
                                <CTableHeaderCell className="text-center">Id inscription</CTableHeaderCell>
                                <CTableHeaderCell className="text-center">Date d&apos;inscription</CTableHeaderCell>
                                <CTableHeaderCell className="text-center">N° borderau</CTableHeaderCell>
                                <CTableHeaderCell className="text-center">Montant</CTableHeaderCell>
                        ...
                        {currInscription.map((inscription,index) => (
                            <CTableRow key={index} >
                                <CTableDataCell> {inscription.id} </CTableDataCell>
                                <CTableDataCell> {inscription.info_borderau.date} </CTableDataCell>
                                <CTableDataCell> {inscription.info_borderau.numero} </CTableDataCell>
                                <CTableDataCell> <CBadge color={currEtudiant.statut_etudiant_re ? "success" : "danger"}>{currEtudiant.statut_etudiant_re ? 'Validé' : 'Non validée'}</CBadge> </CTableDataCell>
                                <CTableDataCell> {inscription.info_borderau.montant} </CTableDataCell>
                                <CTableDataCell> <CLink href={`http://localhost:8000/borderau/${inscription.upload_borderau}`}>{inscription.upload_borderau} </CLink> </CTableDataCell>
       ...
       <CCard className="text-center mt-3 mb-3">
        ...
        {!currEtudiant.statut_etudiant_re && (
